...
def obter_user_supabase(access_token: str):
    ...
    Adiciona logging e tratamento de erro mais robusto.
    """
    ...
    try:
        ...
        # Aumentando o timeout para 15 segundos como segurança contra lentidão
        resp = requests.get(f"{SUPABASE_URL}/auth/v1/user", headers=headers, timeout=15)
        
        if resp.status_code == 200:
            return resp.json()
        
        # Loga falhas de status HTTP
        st.error(f"Falha na validação do token (Status: {resp.status_code}).")
        st.error(f"Resposta bruta do Supabase: {resp.text[:200]}...") # Mostra parte da resposta para debug
...
def sincronizar_usuario(user_json: dict):
    """
    Garante um registro do usuário em 'usuarios'.
    """
    if not user_json:
        return None
    ...
    try:
        ...
        st.error(f"Erro ao sincronizar usuário com o banco de dados 'usuarios': {e}")
        return None
...
# =====================================================
# IA (GPT-5 via Responses API) e EXPORTAÇÃO (Mantidas)
# =====================================================
...
def main():
    ...
    # 1) Tenta converter #access_token em ?access_token (Roda o JS invisível)
    ...
    if "usuario" not in st.session_state:
        ...
        if access_tokens:
            ...
            # 2.1) Valida o token e obtém dados do usuário do Supabase Auth API
            user_json = obter_user_supabase(access_token)
            
            if user_json:
                # 2.2) Sincroniza/cria o registro no banco de dados da sua aplicação
                ...
                if usuario:
                    ...
                    # CRÍTICO: Limpa a query string (remove ?access_token=...)
                    st.experimental_set_query_params() 
                    # CRÍTICO: Força o Streamlit a recarregar no estado de "logado"
                    st.experimental_rerun()
                else:
                    # Falha na sincronização (erro no banco/tabela 'usuarios')
                    st.error("Falha na sincronização do usuário. Verifique as tabelas 'usuarios' e 'etapas' no Supabase.")
                    st.experimental_set_query_params() 
            else:
                # Falha na validação do token (Supabase API)
                st.warning("Não foi possível validar o token de acesso. Verifique os logs de erro acima ou tente novamente.")
                st.experimental_set_query_params() 
            
            # Após tentativas de login (bem-sucedidas ou falhas), exibe a tela de login
            # se não foi feito o rerun para a página principal.
            if "usuario" not in st.session_state:
                tela_login_google()
                return
    ...
    # --- RESTO DA LÓGICA DO APP (Mantida) ---
