...
def atualizar_infos_basicas(projeto_id: int, dados: dict):
    # A correção garante que 'dados' contenha apenas as infos do projeto ativo.
...
"""
    try:
        ...
        return f"Erro ao gerar texto IA (verifique a API Key e o modelo): {e}"
...
def main():
    ...
    with col1:
        ...
        if st.button("Salvar informações básicas"):
            ...
            atualizar_infos_basicas(projeto_id, dados_para_salvar)
            st.success("Informações salvas!")
            st.rerun() # Recarrega para que o objeto 'projeto' seja atualizado na próxima execução
