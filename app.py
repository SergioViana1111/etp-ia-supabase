...
# =====================================================
# FLUXO JS E LEITURA DE SESSION STORAGE (CORREÇÃO DO LOOP)
# =====================================================
...
def obter_token_do_session_storage():
    """Lê o token salvo no sessionStorage via Python Components."""
    
    # 1. Leitor do Token (DEVE ter chave única)
    token = components.html(
        ...
        height=0,
        width=0,
        key="session_storage_reader_return" 
    )
    
    # 2. Removedor do Token (Limpa imediatamente o storage após a leitura)
    if token:
        components.html(
            """<script>sessionStorage.removeItem('supabase_access_token');</script>""",
            height=0,
            width=0,
            key="session_storage_remover_final" 
        )
    return token
