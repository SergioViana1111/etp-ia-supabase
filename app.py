...
def gerar_google_auth_url():
    """
    Gera a URL do OAuth (Supabase) com redirect para o APP_BASE_URL.
    Dica: No Supabase, cadastre nas Redirect URLs as variações com e sem / final.
    """
    if not SUPABASE_URL:
        return "#"
    redirect = (APP_BASE_URL or "http://localhost:8501").rstrip("/")  # normaliza sem barra final
...
def sincronizar_usuario(user_json: dict):
    if not supabase:
        return None
    # Extrai email/nome de forma tolerante
    email = (
        user_json.get("email")
...
def banner_debug_oauth():
    """
    Renderiza um banner visível mostrando se o hash contem access_token
    ANTES de mover o token para a query. Útil para diagnóstico visual.
    """
    components.html(
        """
        ...
        <script>
          (function () {
            try {
              ...
              console.error("Erro banner OAuth:", e);
            }
          })();
        </script>
        """,
...
def mover_access_token_do_hash_para_query():
    """
    Se a URL tiver #access_token=..., move para ?access_token=... e adiciona ?dashboard=1.
    Exibe mensagem no banner e só então faz o replace.
    """
    components.html(
        """
        <script>
        (function () {
          try {
            ...
            console.error("Erro mover hash->query:", e);
          }
        })();
        </script>
        """,
...
def tela_login_google():
    ...
    st.write("URL de autenticação (para debug/copia):")
    ...
    st.markdown(
        ...
        f'font-size:16px;border-radius:6px;cursor:pointer;">🔐 Entrar com Google</button>'
        f'</a>',
        unsafe_allow_html=True
    )

    # Botão alternativo: força navegação via JS (contorna casos onde o link não navega)
    col1, col2 = st.columns([1,3])
    with col1:
        if st.button("Forçar redirect via JS"):
            # Aciona um componente que efetivamente muda window.location
            components.html(
                f"""
                <script>
                  try {{
                    ...
                    console.error("Falha no redirect via JS:", e);
                  }}
                </script>
                """,
                height=0,
            )
            st.stop()  # interrompe o ciclo atual para evitar rerender imediato
...
def main():
    ...
    # 1) Banner e mover hash->query (só exibe/age se houver hash)
