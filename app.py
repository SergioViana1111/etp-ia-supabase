...
def tela_login_google():
    ...
    # CORREÇÃO CRÍTICA: Usar st.link_button ou st.markdown link
    st.link_button(
    ...
    # Nota: Não precisamos de st.session_state["pronto_para_mover_hash"] = True aqui, 
    # pois o link já tira o usuário da página.

    st.caption("Ao clicar, você será redirecionado para o Google/Supabase para autenticação.")

# =====================================================
# FUNÇÕES DE FLUXO JS E LEITURA (MANTIDAS, MAS SÓ SÃO CHAMADAS NO REDIRECIONAMENTO)
# =====================================================
...
def mover_access_token_do_hash_para_session():
    ...
    # Não usamos mais as flags 'pronto_para_mover_hash', mas mantemos a lógica de injeção JS
    components.html(
        """
        <script>
        (function() {
...
def main():
    ...
    # 1) MOVIMENTO DA HASH (SÓ RODA SE A URL TIVER HASH)
    # Roda sempre para capturar o redirecionamento do Supabase
    ...
    if "usuario" not in st.session_state:
        ...
        # 2.1) LEITURA DO SESSION STORAGE
