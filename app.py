...
def tela_login_google():
    ...
    # 1) Link nativo do Streamlit (recomendado)
    try:
        st.link_button("🔐 Entrar com Google (link nativo)", auth_url, help="Vai abrir o fluxo do Supabase/Google")
    except Exception:
        pass  # versões antigas não têm link_button

    # 2) Link de texto (sempre funciona + pode copiar/colar)
    st.write("Se o botão acima não navegar, clique no link de texto abaixo ou copie/cole no navegador:")
    st.write(f"[Abrir login Google (Supabase)]({auth_url})")

    # 3) Exibir a URL crua para debug
